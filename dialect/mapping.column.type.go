...
var gBaseColumnTypeList = []*ColumnTypeInfo{
	...
	{Name: `INTEGER`, Format: `INTEGER`, Matches: []string{`TINYINT`, `SMALLINT`, `MEDIUMINT`, `INT`, `BIGINT`, `BIT&&columnLength==1||columnPrecision==1`, `INT1`, `INT2`, `INT4`, `BOOL`, `BOOLEAN`}, IsNumber: true, IsInteger: true},
	...
	{Name: `NUMERIC`, Format: `NUMERIC($p, $s)`, Matches: []string{`DECIMAL`, `DEC`, `REAL`, `NUMERIC`, `INT8`}, IsNumber: true, IsFloat: true, Comment: `DECIMAL(p,s) 的符合 ANSI 的同义词 p最大精度是38位(十进制)`},
	{Name: `CHAR`, Format: `CHAR($l)`, Matches: []string{`CHARACTER`}, IsString: true},
	{Name: `NCHAR`, Format: `NCHAR($l)`, IsString: true},
