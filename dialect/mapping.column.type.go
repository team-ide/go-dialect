...
Float(n)，数n指示位的精度，可以存储的值的数目。n值的范围可以从 1 到 126。若要从二进制转换为十进制的精度，请将n乘以 0.30103。要从十进制转换为二进制的精度，请用3.32193乘小数精度。126位二进制精度的最大值是大约相当于38位小数精度`},
	...
	{Name: `CLOB`, Format: `CLOB`, Matches: []string{`TINYTEXT`, `TEXT`, `MEDIUMTEXT`, `LONGTEXT`, `ENUM`, `SET`, `LONGVARCHAR`}, IsBytes: true, Comment: `CLOB存储单字节和多字节字符数据。支持固定宽度和可变宽度的字符集。CLOB对象可以存储最多 (4 gigabytes-1) * (database block size) 大小的字符`},
...
var dmColumnTypeList = []*ColumnTypeInfo{
	...
	{Name: `INTEGER`, Format: `INTEGER`, Matches: []string{`MEDIUMINT`, `INT4`}, IsNumber: true, Comment: `用于存储有符号整数，精度为 10，标度为 0。取值范围为：-2147483648(-2^31)～+2147483647(2^31-1)`},
	{Name: `INT`, Format: `INT`, IsNumber: true, Comment: `同上`},
	{Name: `PLS_INTEGER`, Format: `PLS_INTEGER`, IsNumber: true, Comment: `同上`},
	{Name: `BIGINT`, Format: `BIGINT`, Matches: []string{`INT8`}, IsNumber: true, Comment: `用于存储有符号整数，精度为 19，标度为 0。取值范围为：-9223372036854775808(-2^63)～+9223372036854775807(2^63-1)`},
	{Name: `TINYINT`, Format: `TINYINT`, Matches: []string{`INT1`, `BOOL`, `BOOLEAN`}, IsNumber: true, Comment: `储有符号整数，精度为 3，标度为 0。取值范围为：-128～+127`},
	{Name: `SMALLINT`, Format: `SMALLINT`, Matches: []string{`INT2`}, IsNumber: true, Comment: `用于存储有符号整数，精度为 5，标度为 0。取值范围为：-32768(-2^15)~ +32767(2^15-1)`},
	...
	{Name: `FLOAT`, Format: `FLOAT`, Matches: []string{`FLOAT4`}, IsNumber: true, Comment: `FLOAT 是带二进制精度的浮点数，精度最大不超过 53，如省略精度，则二进制精度为 53，十进制精度为 15。取值范围为-1.7E+308～1.7E+308`},
	{Name: `DOUBLE`, Format: `DOUBLE`, Matches: []string{`FLOAT8`}, IsNumber: true, Comment: `同 FLOAT 相似，精度最大不超过 53`},
	...
	{Name: `TIMESTAMP WITH LOCAL TIME ZONE`, Format: `TIMESTAMP WITH LOCAL TIME ZONE`, Comment: `描述一个本地时区的 TIMESTAMP 值，能够将标准时区类型 TIMESTAMP WITH TIME ZONE 类型转化为本地时区类型，如果插入的值没有指定时区，则默认为本地时区。`},
}
...
var kingBaseColumnTypeList = []*ColumnTypeInfo{
	...
	{Name: `BIGINT`, Format: `BIGINT`, Matches: []string{`INT8`}, IsNumber: true, Comment: `有符号整数，取值范围 -9223372036854775808~ +9223372036854775807`},
...
原文链接：https://blog.csdn.net/arthemis_14/article/details/125843469`},
	...
	{Name: `FLOAT`, Format: `FLOAT`, Matches: []string{`DOUBLE`, `FLOAT4`, `FLOAT8`}, IsNumber: true, Comment: `当p取值为1-24时，与REAL相同。当p取值为25-53时，与DOUBLE PRECISION相同。 没有指定精度时，与DOUBLE PRECISION相同`},
	...
	{Name: `CLOB`, Format: `CLOB`, Matches: []string{`TINYTEXT`, `MEDIUMTEXT`, `LONGTEXT`, `SET`, `ENUM`, `NCLOB`, `ROWID`, `UROWID`, `LONGVARCHAR`}, IsBytes: true},
...
var shenTongColumnTypeList = []*ColumnTypeInfo{
	...
	{Name: `INT8`, Format: `INT8`, Matches: []string{`BIGINT`, `NUMBER&&columnScale==0`}, IsNumber: true},
	...
	{Name: `FLOAT8`, Format: `FLOAT8`, Matches: []string{`DOUBLE`, `FLOAT`, `DOUBLE PRECISION`}, IsNumber: true},
	{Name: `CHAR`, Format: `CHAR($l)`, Matches: []string{`NCHAR`, `CHARACTER`}, IsString: true},
	...
	{Name: `CLOB`, Format: `CLOB`, Matches: []string{`TINYTEXT`, `MEDIUMTEXT`, `LONGTEXT`, `ENUM`, `SET`, `NCLOB`, `ROWID`, `UROWID`, `LONGVARCHAR`}, IsBytes: true},
	...
	{Name: `TIMESTAMP`, Format: `TIMESTAMP`, Matches: []string{`YEAR`, `DATETIME`, `TIMESTAMP WITH TIME ZONE`, `TIMESTAMP WITH LOCAL TIME ZONE`, `INTERVAL YEAR TO MONTH`, `INTERVAL DAY TO SECOND`, `DATETIME WITH TIME ZONE`, `TIME WITH TIME ZONE`, `TIMESTAMP WITHOUT TIME ZONE`}},
	{Name: `BOOL`, Format: `BOOL`, IsBoolean: true, Comment: `布尔数据类型：TRUE 和 FALSE。DMSQL 程序的布尔类型和 INT 类型可以相互转化。如果变量或方法返回的类型是布尔类型，则返回值为 0 或 1。TRUE 和非 0 值的返回值为 1，FALSE 和 0 值返回为 0`},
	{Name: `BOOLEAN`, Format: `BOOLEAN`, IsBoolean: true, Comment: `同上`},
}
...
var sqliteColumnTypeList = []*ColumnTypeInfo{
	...
	{Name: `BFILE`, Format: `BFILE`, IsBytes: true},
	{Name: `RAW`, Format: `RAW($l)`, IsBytes: true},
